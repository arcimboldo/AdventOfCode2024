...
class App(app.App):
    ...
    def part_two(self, debug=True):
        ...
        def swap(right, curlen):
            ...
            avail = list(filter(lambda x: x>=curlen and empty_spaces[x], empty_spaces))
            if not avail:
            ...
            for i in range(curlen):
                if self.debug:
                    ...
                    blocks[right+i] = '.'
                adjhash = adjhash + int(self.blocks[right+i])*(left+i) - int(self.blocks[right+i])*(right+i)
            return adjhash
        ...
        if curlen > 0:
            if self.debug:
                self.log(f'LAST -> right {right}, blocks[right]: {blocks[right]} curid: {curid}, curlen: {curlen}')
